%%% Written by Van-Dinh Nguyen and Hieu V. Nguyen
%%% Authors: Van-Dinh Nguyen,  Hieu V. Nguyen, Chuyen T. Nguyen, and Oh-Soon Shin
%%% Corresponding to the paper: "Spectral Efficiency of Full-Duplex Multiuser System: Beamforming Design, 
...
function [ GetBreak, OptimalValue_final, OptValueChain ] = Algorithm2( Pbs, P, H, G_channel, G_SI, G_hat, W_current, p_current, phi_current, rho, Fixed_timegroup_assignment )
...
if (GetBreak)
    ...
    %         alpha_current = alpha_next;
        ...
        %         mu_current = mu_next;
            ...
            DownlinkRate_PerGroupPerUser = DownlinkRate_PerGroupPerUser*diag(time_next);
            ...
            UplinkRate_PerGroupPerUser = UplinkRate_PerGroupPerUser*diag(time_next);
            ...
            if (abs(OptimalValue-OptimalValue_current)<10^-5)
                break;
            else
                OptimalValue_current = OptimalValue
...
function [check] = isFeasible(alpha, beta, time, alpha_next, beta_next, time_next)
...
check1 = [(sum(sum(check_alpha)) + sum(sum(check_beta)) + check_time) == 0];
...
check2 = [(sum(sum(check_alpha_next)) + sum(sum(check_beta_next)) + check_time_next) == 0];

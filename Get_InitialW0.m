function [ W_current ] = Get_InitialW0( H, Pbs, HalfDuplex )
...
for g = 1:1:G
	...
	for k = 1:1:K
		...
		if (HalfDuplex)
            cons_W = [cons_W, real(H(:,k)'*W_next(((k-1)*(N_tx+N_rx)+1):k*(N_tx+N_rx),g))>=0]; % Constraint 12b
        else
            cons_W = [cons_W, real(H(:,k)'*W_next(((k-1)*N_tx+1):k*N_tx,g))>=0]; % Constraint 12b
        end
